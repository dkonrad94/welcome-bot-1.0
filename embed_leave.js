...
let welcomech = tempVars('leave-channel');
let tag = tempVars("tag");
const canvas = Canvas.createCanvas(805, 200);

try {
    var status = config.Leave.Embed.enabled;
} catch {
    var status = false;
}
...
async function welcome() {
    if (status == false) { } else if (tempVars("leave-channel") == undefined) {
        await start();
    } else { }
}
...
async function start() {
    ...
    if (status) {
     ...
     const background = await Canvas.loadImage('./addons/img/leave_bg.png');
     ...
     try {
        ...
        ctx.fillStyle = `${tempVars("config").Leave.Embed.textcolor || "WHITE"}`;
        ctx.fillText(`LEAVE`, canvas.width / 2.0, canvas.height / 3.45);
        ...
        ctx.fillStyle = `${tempVars("config").Leave.TextColor || "WHITE"}`;
        ctx.fillText(`${tempVars("config").Leave.Embed.username.replace('{username}', tempVars("member-username")).replace('{discriminator}', tempVars("member-discriminator"))}`, canvas.width / 3.2, canvas.height / 1.75);
        ...
        ctx.fillStyle = `${tempVars("config").Leave.TextColor || "WHITE"}`;
        ctx.fillText(`${tempVars("config").Leave.Embed.custom_message || "This field is empty."}`, canvas.width / 3.5, canvas.height / 1.20);
        // ------------------------------------------------------------------------------------------------------------------------------------ \\
    } catch (error) {
        console.log(error);
    }
...
// ██║  ██║ ╚████╔╝ ██║  ██║   ██║   ██║  ██║██║  ██║    ██║██║ ╚═╝ ██║██║  ██║╚██████╔╝███████╗
// ╚═╝  ╚═╝  ╚═══╝  ╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝    ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝                                                                                   
    ctx.shadowColor = tempVars("config").Leave.avatarShadow || "WHITE";
...
// ███████╗██║ ╚═╝ ██║██████╔╝███████╗██████╔╝    ██║ ╚═╝ ██║███████╗███████║███████║██║  ██║╚██████╔╝███████╗
// ╚══════╝╚═╝     ╚═╝╚═════╝ ╚══════╝╚═════╝     ╚═╝     ╚═╝╚══════╝╚══════╝╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝                                                                                                     
    const attachment = new Discord.MessageAttachment(canvas.toBuffer(), 'leave_img.png')
    const embed = new Discord.MessageEmbed() 
    .setDescription(`${tempVars("config").Leave.Embed.message.replace('{member}', tempVars("member-ob"))}`)
    .setImage(`attachment://leave_img.png`)
    .setColor(`${tempVars("config").Leave.Embed.color || ""}`)
    .setFooter({ text:`${tempVars("config").Leave.Embed.footer_message.replace('{count}', tempVars("server-members"))}`, iconURL: tempVars("server-icon")})
    .setTimestamp()
    ...
    try {
        welcomech.send({
            ...
            if (tempVars("config").Leave.Emoji.enabled === true) {
            msg.react(`${tempVars("config").Leave.Emoji.leave || ""}`);
        }

        })
    } catch (error) {

        if (tempVars("leave-channel") == undefined) {
            start()
        } else { }
        console.log(error)
    }
